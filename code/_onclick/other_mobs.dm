...
/mob/living/carbon/wereraven/crinos/UnarmedAttack(atom/A, proximity)
	if(HAS_TRAIT(src, TRAIT_HANDS_BLOCKED))
		return
	if(A)
		if(a_intent == INTENT_HARM)
			if(istype(A, /mob/living))
				...
				if(src.tox_damage_plus)
					target.adjustToxLoss(src.tox_damage_plus)
					to_chat(src, "<span class='notice'>Your toxic talons seep into [target]'s flesh!</span>")
				if(src.agg_damage_plus)
					target.adjustCloneLoss(src.agg_damage_plus)
					to_chat(src, "<span class='notice'>Your razor sharp talons rip through [target]'s flesh!</span>")
			return ..()
		A.attack_hand(src)
		update_inv_hands()
