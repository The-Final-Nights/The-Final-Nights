...
/datum/preferences
	...
	//Legacy experience counter per each character's exp total. DO NOT USE THIS ANYMORE.
		...
		if (0) // Character Settings#
			...
			switch(pref_species.name)
				if("Vampire")
					...
					if ((player_experience >= (humanity * 2)) && (humanity < 10))
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("path")
					...
					if ((player_experience < cost) || (humanity >= 10) || !(pref_species.id == "kindred"))
						return
