...
/datum/help_tickets/admin/proc/TicketByID(id)
...
/datum/help_tickets/admin/proc/TicketsByCKey(ckey)
	. = list()
	var/list/lists = list(active_tickets, closed_tickets, resolved_tickets)
	for(var/I in lists)
		for(var/J in I)
...
//private
/datum/help_tickets/admin/proc/ListInsert(datum/admin_help/new_ticket)
...
/datum/help_tickets/admin/proc/BrowseTickets(state)
	var/list/l2b
	var/title
	switch(state)
		if(AHELP_ACTIVE)
...
//Tickets statpanel
/datum/help_tickets/admin/proc/stat_entry()
...
/datum/help_tickets/admin/proc/ClientLogin(client/C)
...
//Dissasociate ticket
/datum/help_tickets/admin/proc/ClientLogout(client/C)
	if(C.current_ticket)
...
/datum/help_tickets/admin/proc/CKey2ActiveTicket(ckey)
	for(var/I in active_tickets)
		var/datum/admin_help/AH = I
		if(AH.initiator_ckey == ckey)
			return AH
