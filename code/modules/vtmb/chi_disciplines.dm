...
/datum/chi_discipline/demon_shintai/activate(mob/living/target, mob/living/carbon/human/caster)
	...
	switch(current_form)
		...
		if("Gods")
			...
			caster.remove_overlay(UNICORN_LAYER)
			var/mutable_appearance/potence_overlay = mutable_appearance('code/modules/wod13/icons.dmi', "holyness1", -UNICORN_LAYER)
			caster.overlays_standing[UNICORN_LAYER] = potence_overlay
			caster.apply_overlay(UNICORN_LAYER)
			caster.physiology.armor.melee += mod
			caster.physiology.armor.bullet += mod
...
/datum/chi_discipline/prayer_eating
	name = "Prayer Eating"
	desc = "Allows you to absorb the energy of prayers directed to you, and transform it into golden chi"
	icon_state = "gods"
	ranged = TRUE
	delay = 12 SECONDS
	discipline_type = "Chi"
...
/datum/action/choose_golden_chim/Trigger()
	if(istype(owner, /mob/living/carbon/human))
		var/mob/living/carbon/human/user = usr
		var/choose_golden_chi = input(user, "Choose what chi you will exchange for your golden essence", "Golden chi") as null|anything in list("Yang", "Yin", "Demon")
		if(user.golden_chi>0)
			...
			to_chat(user, span_warning("You do not have any golden chi to exchange."))
...
/datum/chi_discipline/prayer_eating/activate(mob/living/target, mob/living/carbon/human/caster)
	...
	switch(level_casting)
		if(1)
			...
			new_say = sanitize_text(new_say)
			if (new_say)
				to_chat(caster, span_boldnotice("You transmit to [target]:</span> <span class='notice'>[new_say]</span>"))
				for (var/mob/living/M in oviewers(7, caster))
					to_chat(M, span_boldnotice("You hera a voice from beyond...<span class='notice'>[new_say]</span>"))
		if(2)
        // dot 2: prayer eating
			var/giveprayers = input(target, "[caster] is asking for your prayers, do you do pray to them?", "godly reward") as null|anything in list("accept", "decline")
			switch(giveprayers)
				if("accept")
					to_chat(target, span_userdanger("<b>you feel as energy slowly exits you and enter your god [H]!.</b>"))
					to_chat(caster, span_boldnotice("[target] you feel golden energy, like sweet nectar slowly enter your body as their prayer ends"))
        ...
        // dot 3: fulfill prayer
			...
			switch(god_rewoard)
				if("Power")
					...
					target.physique += 4
					to_chat(target, span_userdanger("<b> YOU FEEL OVERWELMING POWER FLOWING THROUGH YOU, FILLING YOU WITH UNNATURAL STRENGTH.</b>"))
				...
				if("Speed")
					...
					to_chat(target, span_userdanger("<b> YOU FEEL AS IF TIME ITSELF STOPPED AROUND YOU!</b>"))
					spawn((delay)+target.discipline_time_plus)
						if(caster)
							target.remove_movespeed_modifier(/datum/movespeed_modifier/gifted_speed)
							to_chat(target, span_userdanger("<b>you movements start to feel sluggish as your perception returns to normal!.</b>"))
							playsound(target, 'code/modules/wod13/sounds/insanity.ogg', 40, TRUE)
				if("Healing")
		...
		// dot 4: unbreakable soul bond
			var/followering = input(target, "[caster] is asking for you to pray to them, do you do it?", "godly reward") as null|anything in list("accept", "decline")
			if(HAS_TRAIT(target, TRAIT_GAMERGOD))
				...
				switch(followering)
					if("accept")
						to_chat(target, span_userdanger("AS THE ENERGY OF YOUR GOD ENTERS YOUR BODY, YOU ARE FILLED WITH UNWAVERING LOYALTY [H], DO NOT CHALLENGE YOUR GOD, OR YOU WILL SUFFER"))
						to_chat(caster, span_boldnotice("[target] has become your follower!"))
		...
		// dot 5: JUDGEMENT
			if(target.stat >= HARD_CRIT)
				...
				to_chat(target, span_userdanger("<b>as [caster] snaps their fingers, you feel your most vital essence burst out of you!</b>"))
				if(HAS_TRAIT(target, TRAIT_GAMERGOD))
					to_chat(target, span_userdanger("<b>YOU HAVE DISAPOINTED YOUR GOD!</b>"))
