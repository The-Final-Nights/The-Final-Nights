...
/mob/living/carbon/human/proc/drinksomeblood(var/mob/living/mob)
	...
	if(do_after(src, 30, target = mob, timed_action_flags = NONE, progress = FALSE))
		...
		if(clane)
			if(clane.clane_curse_flags & CURSE_GIOVANNI)
				mob.adjustBruteLoss(20, TRUE)
			if((clane.clane_curse_flags & CURSE_VENTRUE) && mob.bloodquality < BLOOD_QUALITY_NORMAL)	//Ventrue can suck on normal people, but not homeless people and animals. BLOOD_QUALITY_LOV - 1, BLOOD_QUALITY_NORMAL - 2, BLOOD_QUALITY_HIGH - 3. Blue blood gives +1 to suction
