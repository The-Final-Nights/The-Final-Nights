...
/obj/machinery/nanite_programmer/ui_act(action, params)
	...
	switch(action)
		...
		if("toggle_active")
			...
			investigate_log("[key_name(usr)] edited [program.name]'s initial activation status into [program.activated ? "Activated" : "Deactivated"]")
			. = TRUE
		if("set_code")
			...
			switch(target_code)
				if("activation")
					...
					investigate_log("[key_name(usr)] edited [program.name]'s activation code into [program.activation_code]")
				if("deactivation")
					program.deactivation_code = clamp(round(new_code, 1),0,9999)
					investigate_log("[key_name(usr)] edited [program.name]'s deactivation code into [program.deactivation_code]")
				if("kill")
					program.kill_code = clamp(round(new_code, 1),0,9999)
					investigate_log("[key_name(usr)] edited [program.name]'s kill code into [program.kill_code]")
				if("trigger")
					program.trigger_code = clamp(round(new_code, 1),0,9999)
					investigate_log("[key_name(usr)] edited [program.name]'s trigger code into [program.trigger_code]")
			. = TRUE
		if("set_extra_setting")
			program.set_extra_setting(params["target_setting"], params["value"])
			investigate_log("[key_name(usr)] edited [program.name]'s extra setting '[params["target_setting"]]' into [params["value"]]")
			playsound(src, "terminal_type", 25, FALSE)
			. = TRUE
		if("set_restart_timer")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name]'s restart timer into [timer/10] s")
			. = TRUE
		if("set_shutdown_timer")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name]'s shutdown timer into [timer/10] s")
			. = TRUE
		if("set_trigger_timer")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name]'s trigger timer into [timer/10] s")
			. = TRUE
		if("set_timer_trigger_delay")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name]'s trigger delay timer into [timer/10] s")
			. = TRUE
