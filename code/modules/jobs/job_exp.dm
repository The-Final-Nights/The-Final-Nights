...
/datum/job/proc/get_exp_req_amount()
	if(title in (GLOB.leader_positions + GLOB.command_positions + GLOB.camarilla_council_positions))
...
/proc/job_is_xp_locked(jobtitle)
	if(!CONFIG_GET(flag/use_exp_restrictions_heads) && (jobtitle in (GLOB.leader_positions + GLOB.command_positions + GLOB.camarilla_council_positions)))
		return FALSE
