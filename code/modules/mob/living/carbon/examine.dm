	...
	if (isgarou(user) || iswerewolf(user))
		if (get_dist(user, src) <= 2)
			...
			if (iskindred(src)) //vampires are static, and may be Wyrm-tainted depending on behaviour
				...
				if ((vampire.clane.clane_curse_flags & CURSE_BAALI) || ( (client?.prefs?.is_enlightened && (vampire.morality_path.score > 7)) || (!client?.prefs?.is_enlightened && (vampire.morality_path.score < 4)) ))
					wyrm_taint++

				if (vampire.clane.clane_curse_flags & CURSE_KIASYD) //the fae are Wyld-tainted by default
					wyld_taint++
