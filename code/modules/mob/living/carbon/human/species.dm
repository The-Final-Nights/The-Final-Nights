...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if(attacker_style?.harm_act(user,target))
		...
		if(atk_verb == ATTACK_EFFECT_KICK)//kicks deal 1.1x raw damage
			target.apply_damage(damage*1.1*meleemod, user.dna.species.attack_type, affecting, armor_block, attack_direction = attack_direction)
			if((damage * 1.5) >= 9)
				target.force_say()
			log_combat(user, target, "kicked")
		else//other attacks deal full damage
			target.apply_damage(damage*meleemod, user.dna.species.attack_type, affecting, armor_block, attack_direction = attack_direction)
			if(damage >= 9)
				target.force_say()
